...
export async function listTodos(opts: {
  ...
  const rows = await db.all<any[]>(
    `
    SELECT id, title, ownerUserId, completed, createdAt
    FROM todos
    ${where}
...
export async function createTodo(
  ...
  await db.run(
    `INSERT INTO todos (id, title, ownerUserId, completed, createdAt) VALUES (?, ?, ?, ?, ?);`,
    id,
    trimmed,
    trimmedUserId,
    0,
  ...
  return { id, title: trimmed, ownerUserId, completed: false, createdAt };
}
...
export async function deleteTodo(
  ...
  await db.run(
    `DELETE FROM todos WHERE id = ? AND ownerUserId = ?;`,
    id,
    owner
  );
