...
function parseQuery(query: unknown): TodosQuery {
  ...
  if (q.includeDeleted !== undefined) {
    if (q.includeDeleted === "true") includeDeleted = true;
    else if (q.includeDeleted === "false") includeDeleted = false;
    else throw new HttpError(400, "includeDeleted must be true or false");
  }
  ...
  return { completed, includeDeleted, limit, offset };
}
...
app.delete("/todos/:id", requireAuth, async (req, res, next) => {
  try {
    ...
    await writeAuditLog({
      ...actor,
      action: "TODO_SOFT_DELETE",
      targetTodoId: deleted.id,
      ip: meta.ip,
      userAgent: meta.userAgent,
...
app.delete(
  ...
  async (req, res, next) => {
    ...
    try {
      const id = req.params.id.trim();
      const result = await softDeleteUserById(id);
