...
describe("P05 Todos API v3 (SQLite)", () => {
  beforeEach(async () => {
    await resetDb();
  });
  ...
  it("POST /todos persists todo with trimmed title and createdAt", async () => {
  ...
  it("PATCH /todos/:id updates title and completed and preserves createdAt", async () => {
    const created = await request(app).post("/todos").send({ title: "A" });
    const id = created.body.todo.id as string;
    const createdAt = created.body.todo.createdAt as string;
    ...
    expect(res.body.todo.createdAt).toBe(createdAt);
  });
  ...
  it("DELETE /todos/:id deletes and persists deletion", async () => {
